import chess
from game_analyzer.pgn_parser import get_fen_matches, classify_opening


def print_result(fen_matches, result):
    print()
...
def test_queens_gambit_declined():
    ...
    result = classify_opening(fen_matches)
    print_result(fen_matches, result)
    assert result["opening_family"] == "Queen's Gambit"
...
def test_kings_indian_classical():
    ...
    result = classify_opening(fen_matches)
    print_result(fen_matches, result)
    assert result["opening_family"] == "Indian Defense"
...
def test_fen_matches_array_grows():
    ...
    result = classify_opening(fen_matches)
    print_result(fen_matches, result)
    assert isinstance(fen_matches, list)
    assert len(fen_matches) >= 2
    assert fen_matches[-1]["name"] == result["opening_line"]
